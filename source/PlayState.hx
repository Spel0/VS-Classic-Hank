...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'bamf':
				{
					...
					var deadSun:FlxSprite = new FlxSprite(1822, 1098).loadGraphic(Paths.image('sunDead', 'week1'));
		...
		switch (curStage)
		{
			...
			case 'happy' | 'evil':
				BF_X = 1466;
				BF_Y = 927;
				if (SONG.player2.startsWith('hank'))
				{
					DAD_X = 409;
					DAD_Y = 750;
				}
				else if (SONG.player2 == 'clown')
				{
					DAD_X = 408;
					DAD_Y = 575;
				}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (SONG.song.toLowerCase() == 'roses' || SONG.song.toLowerCase() == 'thorns' || SONG.song.toLowerCase() == 'crossface')
		{
			remove(black);
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					var useColorSwap = true;
					if ((curStage == 'happy' || curStage == 'evil') && !gottaHitNote) useColorSwap = false;

					var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, useColorSwap, gottaHitNote);
					...
					if(floorSus > 0) {
						for (susNote in 0...floorSus+1)
						{
							...
							var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal((ClientPrefs.noteSpeed != 1 ? ClientPrefs.noteSpeed : SONG.speed), 2)), daNoteData, oldNote, true, false, useColorSwap, gottaHitNote);
